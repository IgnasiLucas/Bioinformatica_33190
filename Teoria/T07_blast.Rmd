...
[<sup>Shawn T. O'Neil, 2021</sup>](https://open.oregonstate.education/computationalbiology/)
...
as.data.frame(M) %>% kable() %>% kable_styling(font_size = 7)
```

## Cambiamos 0 por un valor pequeño {.smaller}
```{r}
MM <- as.data.frame((M + 0.1) / (5 + 0.1 * dim(M)[1]))
MM %>% kable(digits = 2) %>%
...
kable(log2(MM / (1/20)), digits = 2) %>% kable_styling()
```
