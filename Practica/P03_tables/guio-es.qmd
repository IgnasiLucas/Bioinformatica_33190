...
resumen <- data.frame(
   ...
   long.Media = sapply(porTipo, function(x) mean(longitud)),
   long.SD    = sapply(porTipo, function(x)   sd(longitud))
)
...
curl -X POST -H "Content-Type: application/x-www-form-urlencoded" -d 'result=assembly&query=tax_tree(5690)%20AND%20assembly_level%3D%22chromosome%22&fields=assembly_title%2Ctax_id%2Cassembly_level%2Cbase_count%2Ccenter_name%2Ccountry%2Cdepth%2Cgenome_representation%2Clast_updated%2Cscientific_name&format=tsv' "https://www.ebi.ac.uk/ena/portal/api/search"
