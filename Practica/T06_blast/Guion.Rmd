...
Lo primero es asegurarnos de que tenemos `blast+` instalado y que el ordenador sabe
dónde. Para ello, ejecuta el comando siguiente y observa si aparece algún error:

```{r, eval=FALSE}
system2('blastn', args = '-version', stdout = TRUE, stderr = TRUE)
```

Si obtienes un error, puede que no tengas los programas de `ncbi-blast+` instalados,
o bien, que los tengas instalados en una carpeta no incluída en el *PATH* del
ordenador. Si no recuerdas haber instalado `ncbi-blast+`, sigue las instrucciones
de instalación [de este enlace](https://www.ncbi.nlm.nih.gov/books/NBK569861/) y
vuelve a ejecutar el comando anterior. Si el error persiste, entonces indica en
el bloque siguiente la dirección de la carpeta donde lo has instalado y después
ejecútalo para añadir esa carpeta al *PATH* (la lista de carpetas donde RStudio
buscará ejecutables).
...
BLAST_PATH <- 'C:/Archivos de programa/NCBI/blast-BLAST_VERSION+/bin'

if (! grepl(BLAST_PATH, Sys.getenv('PATH')) {
...
Hay dos opciones para descargar las bases de datos que necesitamos. Usa sólo
una de ellas.

### Opción 1
Los programas de `blast+` incluyen un script de Perl que permite descargar o
actualizar las bases de datos de BLAST. Si en tu ordenador tienes un intérprete
de Perl instalado (es improbable), así como los módulos adicionales necesarios,
y si el *script* `update_blastdb.pl` está en el *PATH*, entonces el bloque de
código siguiente debería funcionar.

```{r databases}
# Utilizo "if" para evitar descargar lo mismo cada vez que compilo el documento.
if (! file.exists('16S_ribosomal_RNA.ndb')) {
   system2('update_blastdb',
...
### Opción 2
Si el bloque de código anterior da algún error o no produce ningún resultado,
deberás descargar las bases de datos manualmente y descomprimirlas:

1. Apunta tu navegador a [https://ftp.ncbi.nlm.nih.gov/blast/db/](https://ftp.ncbi.nlm.nih.gov/blast/db/)
2. Descarga los archivos comprimidos `16S_ribosomal_RNA.tar.gz` y `swissprot.tar.gz`.
3. Descomprime los archivos.
4. Para mayor comodidad, mueve todos los archivos de las dos bases de datos a la
   carpeta de trabajo.
...
```{r MIZB01000000}
# Açò no funciona. S'ha d'arreglar.
